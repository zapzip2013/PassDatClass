...
public class Search {
    ...
    //MARK: Initializers
    init(prefix: String, number: String, ver: Bool, order: Sorting = .lastnamealph){
        self.prefix = prefix
        self.number = number
    ...
    func ExecuteQuery() -> [Tutor] {
        var ret = [Tutor]()
        let course = self.prefix + self.number
        let query = "SELECT * FROM tutorBool WHERE bio LIKE '%\(course)%';"//" = '\(course)';"
